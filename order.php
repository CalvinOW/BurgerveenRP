...
<body>
    ...
    <form id="orderform" method="post" target="_self">
        ...
        <select name="sushi_id">
            <option value="">Kies een sushi..</option>
            <?php
            global $pdo;
            ...
            foreach ($result as $data) {
                echo "<option id='$data[id]' value='$data[name]'>" . $data['name'] .  ". Vooraad: " . $data['amount'] . "</option>";
            }
            ?>
        </select> <br>
    ...
    if (isset($_POST['submit'])) {
        if ($_POST['amount'] != 0 && !empty($_POST['amount'])) {
            ...
            if ($amount > $data['amount']) {
                ...
                $sql2 = $pdo->prepare("UPDATE sushi SET amount = $new_amount WHERE id='" . $_POST['id'] . "'");
                $sql2->execute();
